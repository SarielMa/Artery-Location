...
def distanceAvg(ps1, ps2):#nx2 and mx2, all lists
    pairs = []
    pset1 = set()
    pset2 = set()
    ret = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weights_path", type=str, default="checkpoints/yolov3_ckpt_720_160_100.pth", help="path to weights file")
    ...
    for img_i, (path, detections) in enumerate(zip(imgs, img_detections)):
        ...
        base =path.split("test")[0]
        left = base+"labels/"
        right = path.split("test/")[1].split(".")[0]+".json"
        label = left+right
        lumen = []
        ...
        if detections is not None:
            ...
            for x1, y1, x2, y2, cls_conf, cls_pred in detections:
                ...
                """
            distances += distanceAvg(detectedPs,centroids)
            detected += min(len(detectedPs), len(centroids))
