...
import sys
from VW_dataset_doubleWally import VesselSet
import argparse
...
def test(model, device, dataloader):
    ...
    return np.mean(mdis), np.mean(dice_test)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--net_name', default='Resnet18Unet_double', type=str)
    ...
    loader_train = torch.utils.data.DataLoader(dataset=Dataset_train,batch_size = 16, shuffle = True, num_workers=0)
    loader_val = torch.utils.data.DataLoader(dataset=Dataset_val,batch_size = 16, shuffle = False, num_workers=0)
