...
def test(model, device, dataloader):
    ...
    with torch.no_grad():
        for batch_idx, (X, M, L) in enumerate(dataloader):
            
            X, M = X.to(torch.float32).to(device), M.to(torch.float32).to(device)            
            Mp=model(X)
            Mp=torch.sigmoid(Mp)
            Mp = (torch.sign(Mp-0.5)+1)/2                      
            temp=dice(Mp, M)
...
if __name__ == '__main__':
    ...
    loader_val = torch.utils.data.DataLoader(dataset=Dataset_val,batch_size = 32, shuffle = False, num_workers=0)
