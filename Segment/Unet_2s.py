...
class Resnet18Unet(nn.Module):
    def __init__(self, n_point, n_seg, flag=0, offset=0, output='seg'):
        super().__init__()
        ...
        self.e4 = nn.Sequential(Block(512, 1024, 2), Block(1024, 1024, 1),
                                nn.Conv2d(1024, 1024, 2, 1, 0, bias=False))# here output N*1024*11*11
        
        self.e5 = nn.Sequential(nn.Flatten(),
                                nn.GroupNorm(1, 1024*121),
                                nn.LeakyReLU(inplace=True),
                                nn.Linear(1024*121, 1024, bias=False),
                                nn.GroupNorm(1, 1024),
                                nn.LeakyReLU(inplace=True),
...
class Resnet18Unet_old(nn.Module):
    def __init__(self, n_point, n_seg, flag=0, offset=0, output='seg'):
        super().__init__()
        ...
        self.g3 = nn.Sequential(nn.Conv2d(1024, 256, 3, 1, 1, bias=False),
                                nn.GroupNorm(4, 256),
                                nn.LeakyReLU(inplace=True),
                                nn.ConvTranspose2d(256, 256, 3, 2, 1, 0, bias=False),
                                nn.GroupNorm(4, 256),
                                nn.LeakyReLU(inplace=True))
